...
-- Result: VAT | Holy See (Vatican City State)
...
-- SELECT code, name, countrycode, language FROM country LEFT JOIN countrylanguage ON code = countrycode WHERE language = 'Italian' AND isofficial = 't';
-- SELECT * FROM countrylanguage WHERE countrycode = 'SMR';
-- Result: San Marino (SMR)
...
-- SELECT code, continent, region, c.name, countrycode FROM country JOIN city c ON code = countrycode WHERE c.name LIKE "%Serra";
-- Result: Serra, Brazil (BRA)

-- Clue #6: We're close! Our South American agent says she just got a taxi at the airport, and is headed towards the capital! Look up the country's capital, and get there pronto! Send us the name of where you're headed and we'll follow right behind you!

-- SELECT co.name, capital, ci.id, ci.name FROM country co JOIN city ci ON capital = ci.id WHERE co.name = 'Brazil';
-- Result: Brasilia
